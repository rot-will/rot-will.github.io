(() => {
  ...
  const tocstorageName= 'hexo-blog-encrypt.toc:#' + window.location.pathname;
...
// so adding an empty div and check it when decrption.
  ...
  const tocData=document.getElementById('toc').innerHTML;
  ...
  async function decrypt(decryptKey, iv, hmacKey) {
    ...
    const result = await cryptoObj.subtle.decrypt({
      ...
      document.getElementById('toc').innerHTML=tocData;
